...
resource "null_resource" "remote-exec_tomcat-2" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "chmod +x ~/*.sh",
      "./config_tomcat_ords.sh \"${var.target_db_admin_pw}\" ${var.target_db_ip} ${var.target_db_srv_name} ${var.target_db_public_ip} ${var.com_port} ${var.URL_ORDS_file}",
    ]
  }
}
...
resource "null_resource" "remote-exec_tomcat-apex" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "./config_tomcat_apex.sh \"${var.target_db_admin_pw}\" ${var.target_db_ip} ${var.target_db_srv_name} ${var.target_db_public_ip} ${var.com_port} ${var.URL_APEX_file} ${var.APEX_install_mode}",
    ]
  }
}
